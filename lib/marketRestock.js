...
//
// Cada publicación es de a 1 unidad -- el mercado (routes/market.js) no soporta "comprar 3 de un
// lote de 15", /listings/:id/buy vende la publicación entera. Por eso se mantienen hasta
// STOCK_TARGET publicaciones sueltas por poción en vez de una sola con quantity=15.
...
// ACTIVE, repone hasta STOCK_TARGET. Se llama periódicamente desde server.js (mismo patrón que
// tickWorldBossSchedule) y una vez al arrancar.
async function restockPotions() {
  ...
  for (const code of POTION_CODES) {
    ...
    const toCreate = STOCK_TARGET - activeCount;
    for (let i = 0; i < toCreate; i++) {
      await db.query(
        `INSERT INTO player_market_listings(seller_id, item_id, enchant_level, quality_tier, quantity, price_per_unit, currency)
         VALUES ($1, $2, 0, 0, 1, $3, 'GOLD')`,
        [sellerId, item.id, PRICE_PER_UNIT]
      );
    }
  }
}

module.exports = { restockPotions, getSystemSellerId, SYSTEM_SELLER_EMAIL, POTION_CODES };
