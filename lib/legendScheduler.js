...
async function actorHasFarmableUpgrade(playerId, zoneId, { classId, level, npcId }) {
  const classChain = await evolution.getClassAncestorChain(classId);
  const candidates = (await db.query(
    `SELECT cr.id AS recipe_id, cr.result_item_id, i.slot
     FROM crafting_recipes cr JOIN items i ON i.id = cr.result_item_id
     WHERE cr.zone_id = $1 AND i.item_type = 'EQUIPMENT' AND i.slot IS NOT NULL
       AND (i.class_id IS NULL OR i.class_id = ANY($2::int[]))
       AND (i.required_level IS NULL OR i.required_level <= $3)
       AND (cr.scroll_item_id IS NULL OR EXISTS (SELECT 1 FROM player_learned_recipes plr WHERE plr.player_id = $4 AND plr.recipe_id = cr.id))`,
    [zoneId, classChain, level, playerId]
  )).rows;
  ...
  for (const c of candidates) {
    ...
    for (const ing of ingredients) {
      if ((await inventory.getQuantity(playerId, ing.item_id)) < ing.quantity) return true;
    }
  }
  return false;
}

// NPCs primero, héroe después (mismo orden que craftUpgradeFor, no que importe acá al no gastar
// nada, pero mantiene el mismo patrón de lectura en todo el archivo).
async function zoneHasFarmableUpgrade(playerId, zoneId, ctx, npcs) {
  for (const npc of npcs) {
    if (await actorHasFarmableUpgrade(playerId, zoneId, { classId: npc.class_id, level: npc.level, npcId: npc.id })) return true;
  }
  const heroClassId = ctx.evolution_class_id || ctx.current_class_id;
  return actorHasFarmableUpgrade(playerId, zoneId, { classId: heroClassId, level: ctx.level });
}

// Pedido explícito del usuario: que las leyendas no avancen de zona por nivel sin más -- que
// primero terminen de craftear y equipar el mejor set posible de la zona más baja pendiente (héroe
// Y sus NPCs), farmeando ahí los materiales que les falten, y recién ahí sigan a la siguiente. Antes
...
async function pickZoneToExplore(playerId, ctx) {
  ...
  for (const z of zones) {
    if (await zoneHasFarmableUpgrade(playerId, z.id, ctx, npcs)) return z.id;
  }
