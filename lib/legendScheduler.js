...
// puntual es siempre el mismo -- cachearlo en memoria evita repetir la misma query miles de veces
// por tick una vez que pickZoneToExplore (más abajo) empieza a puntuar candidatos de receta por
// zona: sin esto, cada leyenda avanzada terminaría re-escaneando el mismo puñado de ítems de zonas
// ya completas en CADA tick, para siempre, contra un pool de Supabase que ya viene justo de
// conexiones (ver comentario de POOL_MAX en db/db.js).
const itemScoreCache = new Map();

async function scoreItem(itemId, enchantLevel = 0, qualityTier = 0) {
  const cacheKey = `${itemId}:${enchantLevel}:${qualityTier}`;
