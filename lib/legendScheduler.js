...
const LEGEND_TICK_INTERVAL_MS = 90 * 60 * 1000; // ~90 min por leyenda
...
async function tryHeal(playerId, ctx) {
  ...
  if (Number(ctx.gold) >= 1) {
    const r = await callRoute(H.guildHeal, playerId, { body: { heroOnly: true } });
    if (r.status < 400) return true;
  }
  return false;
}
...
async function processLegendTick(playerId) {
  ...
  // Si esta critica, intenta curarse y listo por este tick. Pero si NO se pudo curar (sin oro ni
  // pociones -- puede pasar tras una mala racha), no cortar del todo: nada más abajo de acá
  // arriesga otra pelea salvo daily-event/explorar (esos sí quedan bloqueados), así que igual
  // puede reclamar recompensa diaria, aceptar/completar misión, craftear, etc. -- caminos reales
  // para juntar oro y curarse en un tick futuro en vez de quedar trabada para siempre.
  const critical = Number(ctx.hp) <= 0 || Number(ctx.hp) / Number(ctx.max_hp) < CRITICAL_HP_RATIO;
  if (critical && (await tryHeal(playerId, ctx))) return;
  ...
  if (!critical && (await tryDailyEvent(playerId, ctx))) return;
  ...
  if (!critical) await exploreAndFight(playerId, ctx);
}
