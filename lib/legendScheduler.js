...
const STAT_WEIGHT = {
  DEF: 0.5, MAGIC_DEF: 0.5, SPD: 0.3,
  CRIT_CHANCE: 2, CRIT_DAMAGE: 1, EVASION: 1, HP: 0.1, LUCK: 1,
};
// Pedido explícito del usuario: antes ATK y MAG pesaban 1 cada uno sin mirar la clase de quien se
// va a equipar -- un Guerrero (físico) podía terminar "prefiriendo" un ítem con más MAG que ATK
// solo porque el número era más grande, aunque el MAG no le sirviera para nada (ninguna de sus
// skills escala con eso). PRIMARY_STAT_WEIGHT/OFF_STAT_WEIGHT reemplazan el peso fijo de 1: el
// stat que de verdad usa la clase (ATK para físicas, MAG para mágicas) pesa mucho más que el otro,
// que no queda en 0 nomás por si algún ítem/skill híbrido igual se beneficia un poco.
const PRIMARY_STAT_WEIGHT = 1.5;
const OFF_STAT_WEIGHT = 0.3;
const QUALITY_TIER_MULTIPLIER = [1.0, 1.15, 1.35, 1.60, 2.0];

// classes.base_atk vs base_mag ya distingue las 5 clases base (y sus evoluciones heredan el mismo
// perfil) -- no hace falta una columna nueva "es física/mágica", esa relación ya está en los datos.
// Cacheado: son ~94 filas fijas, no cambian en runtime.
const classAffinityCache = new Map();
async function getClassAffinity(classId) {
  if (classAffinityCache.has(classId)) return classAffinityCache.get(classId);
  const row = (await db.query('SELECT base_atk, base_mag FROM classes WHERE id = $1', [classId])).rows[0];
  const affinity = row && Number(row.base_mag) > Number(row.base_atk) ? 'MAG' : 'ATK';
  classAffinityCache.set(classId, affinity);
  return affinity;
}

function statWeightFor(statCode, classAffinity) {
  if (statCode === 'ATK') return classAffinity === 'ATK' ? PRIMARY_STAT_WEIGHT : OFF_STAT_WEIGHT;
  if (statCode === 'MAG') return classAffinity === 'MAG' ? PRIMARY_STAT_WEIGHT : OFF_STAT_WEIGHT;
  return STAT_WEIGHT[statCode] || 0;
}

// item_stat_bonuses no cambia en runtime, así que el puntaje de un (item, encantamiento, calidad,
// afinidad) puntual es siempre el mismo -- cachearlo en memoria evita repetir la misma query miles
// de veces por tick una vez que pickZoneToExplore (más abajo) empieza a puntuar candidatos de
// receta por zona: sin esto, cada leyenda avanzada terminaría re-escaneando el mismo puñado de
// ítems de zonas ya completas en CADA tick, para siempre, contra un pool de Supabase que ya viene
// justo de conexiones (ver comentario de POOL_MAX en db/db.js).
const itemScoreCache = new Map();

async function scoreItem(itemId, enchantLevel = 0, qualityTier = 0, classAffinity = 'ATK') {
  const cacheKey = `${itemId}:${enchantLevel}:${qualityTier}:${classAffinity}`;
  if (itemScoreCache.has(cacheKey)) return itemScoreCache.get(cacheKey);
  const bonuses = await db.query('SELECT stat_code, amount FROM item_stat_bonuses WHERE item_id = $1', [itemId]);
  let score = 0;
  for (const b of bonuses.rows) score += Number(b.amount) * statWeightFor(b.stat_code, classAffinity);
  score *= QUALITY_TIER_MULTIPLIER[qualityTier] ?? 1;
  score *= 1 + Number(enchantLevel) * 0.05;
  itemScoreCache.set(cacheKey, score);
  return score;
}

async function scoreEquippedSlot(playerId, slot, classAffinity) {
  const row = (await db.query(
    'SELECT item_id, enchant_level, quality_tier FROM player_equipment WHERE player_id = $1 AND slot = $2',
    [playerId, slot]
  )).rows[0];
  if (!row) return 0;
  return scoreItem(row.item_id, row.enchant_level, row.quality_tier, classAffinity);
}

async function scoreNpcEquippedSlot(npcId, slot, classAffinity) {
  const row = (await db.query(
...
// Pedido explícito del usuario: el puntaje de mascota tampoco miraba la clase del héroe -- sumaba
// TODOS los bonos de stat por igual (ATK_FLAT y MAG_FLAT valían lo mismo), así que una mascota con
// +MAG podía "ganarle" a una con +ATK aunque el MAG no le sirviera de nada a un Guerrero. Mismo
// criterio de PRIMARY_STAT_WEIGHT/OFF_STAT_WEIGHT que ya usa scoreItem para equipo -- el resto de
// los bonos de mascota (oro%, xp%, reducción de daño%, etc.) quedan sin tocar, a peso 1, esto solo
// corrige específicamente el caso ATK vs MAG.
const PET_CLASS_STAT = { ATK: 'ATK_FLAT', MAG: 'MAG_FLAT' };
const PET_CLASS_DAMAGE_STAT = { ATK: 'PHYSICAL_DAMAGE_PERCENT', MAG: 'MAGICAL_DAMAGE_PERCENT' };

async function scorePlayerPets(playerId, classAffinity) {
  const rows = (await db.query(
    `SELECT pp.id, pp.is_active, pb.stat_code, (pb.base_amount + pb.per_level_amount * (pp.level - 1)) AS amount
     FROM player_pets pp LEFT JOIN pet_bonuses pb ON pb.pet_id = pp.pet_id
     WHERE pp.player_id = $1`,
    [playerId]
  )).rows;

  const offAffinity = classAffinity === 'ATK' ? 'MAG' : 'ATK';
  const petsById = new Map();
  for (const r of rows) {
    if (!petsById.has(r.id)) petsById.set(r.id, { id: r.id, is_active: r.is_active, score: 0 });
    if (!r.stat_code) continue;
    let weight = 1;
    if (r.stat_code === PET_CLASS_STAT[classAffinity] || r.stat_code === PET_CLASS_DAMAGE_STAT[classAffinity]) {
      weight = PRIMARY_STAT_WEIGHT;
    } else if (r.stat_code === PET_CLASS_STAT[offAffinity] || r.stat_code === PET_CLASS_DAMAGE_STAT[offAffinity]) {
      weight = OFF_STAT_WEIGHT;
    }
    petsById.get(r.id).score += Number(r.amount) * weight;
  }
  return [...petsById.values()];
}

async function maybeEquipBetterPet(playerId, newPlayerPetId, classAffinity) {
  const pets = await scorePlayerPets(playerId, classAffinity);
...
// Reconsidera TODAS las mascotas ya obtenidas (no solo la recién nacida) contra la activa actual --
// hace falta aparte de maybeEquipBetterPet porque esa solo compara "la nueva contra la activa" en
// el momento de nacer; sin esto, una mascota vieja que quedó sin activar bajo el puntaje viejo (sin
// afinidad de clase) nunca se reconsidera después.
async function tryActivateBestOwnedPet(playerId, classAffinity) {
  const pets = await scorePlayerPets(playerId, classAffinity);
  if (!pets.length) return false;
  const current = pets.find((p) => p.is_active);
  const best = pets.reduce((b, p) => (p.score > b.score ? p : b), pets[0]);
  if (current && best.id === current.id) return false;
  const r = await callRoute(H.petActivate, playerId, { params: { playerPetId: best.id } });
  return r.status < 400;
}

async function tryPetTasks(playerId, classAffinity) {
  ...
  if (inc.body && inc.body.ready) {
    ...
    if (claim.status < 400 && claim.body?.id) await maybeEquipBetterPet(playerId, claim.body.id, classAffinity);
    return true;
  }
  if (!inc.body) {
  ...
  return tryActivateBestOwnedPet(playerId, classAffinity);
}
...
async function equipBestOwnedFor(playerId, { classId, level, npcId }) {
  ...
  const classAffinity = await getClassAffinity(classId);
  ...
  for (const o of owned) {
    ...
    const score = await scoreItem(o.item_id, o.enchant_level, o.quality_tier, classAffinity);
    const prev = bestBySlot.get(o.slot);
    if (!prev || score > prev.score) bestBySlot.set(o.slot, { ...o, score });
  }

  for (const [slot, best] of bestBySlot) {
    const currentScore = npcId ? await scoreNpcEquippedSlot(npcId, slot, classAffinity) : await scoreEquippedSlot(playerId, slot, classAffinity);
...
async function craftUpgradeFor(playerId, { classId, level, npcId }) {
  ...
  for (const c of candidates) {
    ...
    const newScore = await scoreItem(c.result_item_id, 0, 0, classAffinity);
    const currentScore = npcId ? await scoreNpcEquippedSlot(npcId, c.slot, classAffinity) : await scoreEquippedSlot(playerId, c.slot, classAffinity);
...
async function tryBuyGuildShopUpgrade(playerId, ctx) {
  ...
  const classAffinity = await getClassAffinity(ctx.evolution_class_id || ctx.current_class_id);
  for (const item of items) {
    if (!item.affordable) continue;
    if (Number(shop.body.gold) - item.buyPrice < GOLD_RESERVE_FOR_HEALING) continue;
    const newScore = await scoreItem(item.id, 0, 0, classAffinity);
    const currentScore = await scoreEquippedSlot(playerId, item.slot, classAffinity);
    if (newScore <= currentScore) continue;
...
// ¿Le queda a este actor (héroe o NPC puntual) alguna mejora de equipo por FARMEAR en esta zona
// puntual? Mismo criterio de elegibilidad de receta que craftUpgradeFor (clase/nivel del actor,
// receta ya aprendida si pide scroll), pero acotado a cr.zone_id = zoneId en vez de "cualquier zona
// ya desbloqueada", y invertido: solo cuenta si el resultado puntúa MEJOR que lo puesto Y todavía
// FALTA al menos un material. Si ya tuviera el material completo, craftUpgradeFor (que corre antes
// en la cola, ver processLegendTick) ya la habría crafteado y equipado ese mismo tick -- llegar
// hasta acá con los materiales completos no debería pasar en la práctica.
// Devuelve el conjunto de item_id que le faltan para completar CUALQUIER mejora de equipo
// pendiente (misma clase/nivel del actor, ya aprendida si pide scroll, puntúa mejor que lo
// puesto), sin importar en qué zona esté matriculada la receta -- lo que importa después es
// DÓNDE DROPEA DE VERDAD el material que falta, no la zona "nominal" de la receta. Confirmado en
// producción que no siempre coinciden: "Peto de Cuero Reforzado" está matriculado en Pradera
// Dorada pero uno de sus ingredientes (Mineral de Hierro) solo dropea en Montañas Grises -- la
// version anterior de esto asumía que coincidían, y Kael (nivel 18) quedó atrapado farmeando
// Pradera Dorada para siempre por esa receta puntual, sin poder avanzar de zona nunca.
async function actorMissingIngredients(playerId, { classId, level, npcId }) {
  const classChain = await evolution.getClassAncestorChain(classId);
  const classAffinity = await getClassAffinity(classId);
  const candidates = (await db.query(
    `SELECT cr.id AS recipe_id, cr.result_item_id, i.slot
     FROM crafting_recipes cr JOIN items i ON i.id = cr.result_item_id
     WHERE i.item_type = 'EQUIPMENT' AND i.slot IS NOT NULL
       AND (i.class_id IS NULL OR i.class_id = ANY($1::int[]))
       AND (i.required_level IS NULL OR i.required_level <= $2)
  ...
  // Mismo chequeo que craftUpgradeFor: un arma a dos manos ocupa el OFFHAND también, no tiene
  // sentido seguir farmeando materiales de un accesorio que después ni se va a poder equipar.
  const weaponRow = (await db.query(
    npcId
      ? `SELECT i.is_two_handed FROM npc_equipment ne JOIN items i ON i.id = ne.item_id WHERE ne.npc_id = $1 AND ne.slot = 'WEAPON'`
      : `SELECT i.is_two_handed FROM player_equipment pe JOIN items i ON i.id = pe.item_id WHERE pe.player_id = $1 AND pe.slot = 'WEAPON'`,
  ...
  for (const c of candidates) {
    ...
    const ingredients = (await db.query(
      'SELECT item_id, quantity FROM crafting_recipe_ingredients WHERE recipe_id = $1', [c.recipe_id]
    )).rows;
    for (const ing of ingredients) {
      if ((await inventory.getQuantity(playerId, ing.item_id)) < ing.quantity) missing.add(ing.item_id);
    }
  }
  return missing;
}

// Pedido explícito del usuario: que las leyendas no avancen de zona por nivel sin más -- que
// primero terminen de craftear y equipar el mejor set posible (héroe Y sus NPCs), farmeando los
// materiales que les falten donde de verdad dropean, y recién ahí sigan a la siguiente zona. Antes
// esto era pickZoneForLevel: mandaba a explorar directo a la zona del nivel del héroe sin mirar el
// equipo -- así una leyenda de nivel 16 ya exploraba Montañas Grises con el gear base "de Pradera"
// puesto y sus NPCs con los 7 slots vacíos, confirmado en producción (Kael/Sylvan/Vorak).
//
// Los ingredientes faltantes se calculan UNA sola vez (no por zona, no dependen de la zona) y
// después se chequea barato, zona por zona, si alguno de esos ítems dropea ahí -- mucho más
// eficiente que la version anterior (recalculaba lo mismo por cada zona candidata) y además
// corrige el bug de arriba.
async function pickZoneToExplore(playerId, ctx) {
  const zones = (await db.query(
    `SELECT id FROM monster_zones WHERE is_tower_zone = FALSE AND min_level <= $1 ORDER BY min_level ASC`,
    [ctx.level]
  )).rows;
  ...
  const missing = new Set();
  for (const npc of npcs) {
    for (const id of await actorMissingIngredients(playerId, { classId: npc.class_id, level: npc.level, npcId: npc.id })) missing.add(id);
  }
  const heroClassId = ctx.evolution_class_id || ctx.current_class_id;
  for (const id of await actorMissingIngredients(playerId, { classId: heroClassId, level: ctx.level })) missing.add(id);

  if (missing.size) {
    const missingIds = [...missing];
    for (const z of zones) {
      const dropsHere = (await db.query(
        `SELECT 1 FROM monster_drops md JOIN monsters m ON m.id = md.monster_id WHERE m.zone_id = $1 AND md.item_id = ANY($2::int[]) LIMIT 1`,
        [z.id, missingIds]
      )).rows;
      if (dropsHere.length) return z.id;
    }
  }
...
// Pedido explícito del usuario originalmente: escapar si el enemigo es de nivel mayor al del
// héroe -- una pelea de zona puede tocar un monstruo por encima del propio nivel (confirmado:
// Luminus nivel 1 contra un Bandido Corrupto/Araña Venenosa Lv.2, pelea perdida del todo pese a
// tener curador). Solo aplica cuando le toca el turno al HÉROE (ESCAPE es la única accion que el
// motor le rechaza a un NPC) y se revisa antes que la curación -- no tiene sentido curarse para
// seguir en una pelea que de entrada convenía evitar.
//
// OVERLEVEL_RATIO (no "cualquier diferencia de nivel") -- confirmado en producción que la
// version original ("enemigo.nivel > heroe.nivel", sin más) rompía peleas totalmente ganables:
// el Titán de la Pradera sortea su nivel entre 14 y 15, y con un héroe nivel 14, la mitad de las
// veces el motor lo hacía intentar escapar TODOS los turnos (y fallar) en vez de pelear, dejando
// el daño real entero en manos de los NPCs solos -- Elyndor nunca pudo ganarle por esto mismo
// mientras Kael sí, solo porque le tocó más seguido la variante de nivel 14. 1 nivel de diferencia
// es enorme en términos relativos a nivel 1 (casi sin stats) pero insignificante a nivel 14 (ya con
// equipo completo +2) -- por eso el umbral es relativo al nivel del héroe, no un número fijo.
const OVERLEVEL_RATIO = 0.2; // huye si el enemigo supera al héroe en más de un 20% de su propio nivel
...
// isWorldBoss: pedido explícito del usuario -- el World Boss (500.000 HP) está diseñado para
// perder siempre con un solo clon, el punto es el daño hecho (reparte cosmic_shards igual, ver
// handleWorldBossFinalize), no sobrevivir. Curarse ahí (con poción O con skill) solo alarga la
// pelea sin cambiar el resultado, y si el grupo nunca llega a HP 0 la sesión no se resuelve sola
// -- termina forzando un ESCAPE en playOutCombat, que sí pierde el crédito del daño. Sin curación
// de ningún tipo, el grupo cae naturalmente bastante antes del tope de acciones, así que la pelea
// siempre termina en ENEMY_WON (con crédito) en vez de arrastrarse hasta un escape forzado.
async function decideCombatAction(playerId, actor, allies, enemies, heroLevel, isWorldBoss = false) {
  if (actor.player_id === playerId && enemies?.length) {
    const overLeveled = enemies.some((e) => {
      const lvl = parseMonsterLevel(e.name);
      return lvl !== null && lvl > heroLevel && (lvl - heroLevel) / heroLevel > OVERLEVEL_RATIO;
    });
  ...
  if (!isWorldBoss && neediest && Number(neediest.hp) / Number(neediest.max_hp) < ALLY_HEAL_HP_RATIO) {
...
async function playOutCombat(playerId, sessionId, heroLevel, isWorldBoss = false) {
  let guard = 0;
  while (guard++ < MAX_FIGHT_ACTIONS) {
    ...
    const action = await decideCombatAction(playerId, actor, allies, aliveEnemies, heroLevel, isWorldBoss);
  ...
  // Pedido explícito del usuario: el World Boss nunca debe escapar -- sin curación de ningún tipo
  // (ver isWorldBoss en decideCombatAction), el grupo cae solo mucho antes de llegar acá, así que
  // este fallback ni debería alcanzar a ejecutarse para World Boss en la práctica. Si por algún
  // motivo igual llegara (enemigo débil, mala suerte de rondas), mejor dejar la sesión como esté
  // (el próximo tick la recupera vía recoverStaleSession a los 10 min) que escapar y perder el
  // crédito del daño ya hecho.
...
async function tryWorldBoss(playerId, ctx) {
  ...
  if (enter.status >= 400 || !enter.body?.session) return false;
  await playOutCombat(playerId, enter.body.session.id, ctx.level, true);
  return true;
}
...
async function exploreAndFight(playerId, ctx) {
  const zoneId = await pickZoneToExplore(playerId, ctx);
...
async function processLegendTick(playerId) {
  ...
  try {
    ...
    if (await tryPetTasks(playerId, await getClassAffinity(ctx.evolution_class_id || ctx.current_class_id))) return;
