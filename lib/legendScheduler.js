...
// ---------- 8.5) Craftear pociones de HP si le quedan pocas ----------
// No hay ninguna tienda del juego que venda pociones ni sus ingredientes (verificado contra
// artisan_shop/tower_vendor_shop/etc.) -- vienen solo de drops o de craftearlas con lo que ya
// juntó peleando, así que esto es lo único real que se puede hacer para reponer stock.
const POTION_RECIPE_CODES = ['RECETA_POCION_DE_VIDA_MENOR', 'RECETA_POCION_DE_VIDA_MAYOR'];
const MIN_POTION_STOCK = 3;

async function tryCraftPotion(playerId) {
  const haveRow = (await db.query(
    `SELECT COALESCE(SUM(pi.quantity), 0) AS total FROM player_inventory pi
     JOIN item_stat_bonuses isb ON isb.item_id = pi.item_id AND isb.stat_code = 'HEAL_HP'
     WHERE pi.player_id = $1`,
    [playerId]
  )).rows[0];
  if (Number(haveRow.total) >= MIN_POTION_STOCK) return false;

  for (const code of POTION_RECIPE_CODES) {
...
// target_type=ALLY puede apuntar a cualquiera) por debajo de ALLY_HEAL_HP_RATIO y el actor tiene
// con que curar, cura al mas necesitado -- no solo a si mismo, para que una skill de sanacion real
// sirva para lo que existe. Si esta muy mal (<25%) y no se pudo curar, mejor escapar (no siempre
// sale, ver combat.escapeChance, pero un intento fallido cuesta lo mismo que igual seguir
// atacando) que seguir peleando a muerte -- salvo que el enemigo mas debil tambien este casi
// muerto, ahi vale la pena arriesgar el ultimo golpe en vez de huir de una pelea ya ganada. ESCAPE
// solo lo puede pedir el héroe (nunca un NPC, el motor lo rechaza con 400) -- un NPC bajo de HP
// simplemente sigue atacando/curando. Si no aplica nada de lo anterior, ataca con la skill mas
// cara que le alcance la mana (proxy simple de "mas fuerte", termina la pelea mas rapido que el
// golpe basico), o basico si no tiene ninguna.
async function decideCombatAction(playerId, actor, enemies, allies) {
...
async function playOutCombat(playerId, sessionId) {
  ...
  while (guard++ < MAX_FIGHT_ACTIONS) {
    ...
    const action = await decideCombatAction(playerId, actor, aliveEnemies, allies);
