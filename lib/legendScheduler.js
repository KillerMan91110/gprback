...
// nuevo, solo usarlo.
//
// Pedido explícito del usuario: el equipo (héroe + 2 NPCs) siempre tiene que tener un curador -- de
// las 5 clases base, solo Sacerdote tiene una skill de tipo CURACION (la aprende ya en nivel 1,
// automática, ver decideCombatAction). Si ni el héroe ni ningún NPC del grupo es Sacerdote, prioriza
// contratar uno del pool por sobre "el más caro que pueda pagar"; si el grupo ya está lleno de 2 NPCs
// sin curador, despide al menos útil (preferí uno de clase duplicada dentro del grupo, ej. 2 Pícaros)
// y lo reemplaza por un Sacerdote en el MISMO tick, pero solo si hay uno contratable en el pool
// ahora mismo -- así nunca deja un slot vacío esperando (eso repetiría el despido cada tick sin
// necesidad, en vez de simplemente esperar a que el pool se refresque con uno disponible).
const PARTY_MAX_NPC_SLOTS = 2;
const HEALER_CLASS_CODE = 'SACERDOTE'; // classes.code
const HEALER_CLASS_NAME = 'Sacerdote'; // classes.name, como llega en /party/pool

function pickNpcToReplace(npcs) {
  const counts = {};
  for (const n of npcs) counts[n.class_code] = (counts[n.class_code] || 0) + 1;
  return npcs.find((n) => counts[n.class_code] > 1) || npcs[0];
}

async function tryRecruitNpc(playerId, ctx) {
  const heroClassId = ctx.evolution_class_id || ctx.current_class_id;
  const heroClassCode = (await db.query('SELECT code FROM classes WHERE id = $1', [heroClassId])).rows[0]?.code;

  const partyRows = (await db.query(
    `SELECT pp.id AS party_row_id, c.code AS class_code
  ...
  const hasHealer = heroClassCode === HEALER_CLASS_CODE || partyRows.some((n) => n.class_code === HEALER_CLASS_CODE);

  const pool = await callRoute(H.partyPool, playerId);
  const npcs = pool.body?.npcs;
  if (!Array.isArray(npcs) || !npcs.length) return false;
  const affordable = npcs.filter((n) => Number(n.hireCost) <= Number(ctx.gold));
  const healerPick = affordable.find((n) => n.className === HEALER_CLASS_NAME);

  if (partyRows.length < PARTY_MAX_NPC_SLOTS) {
    const pick = (!hasHealer && healerPick) || affordable.sort((a, b) => b.hireCost - a.hireCost)[0];
    if (!pick) return false;
    const r = await callRoute(H.partyHire, playerId, { params: { poolNpcId: String(pick.poolNpcId) } });
    return r.status < 400;
  }

  if (!hasHealer && healerPick) {
    const toReplace = pickNpcToReplace(partyRows);
    ...
    const r = await callRoute(H.partyHire, playerId, { params: { poolNpcId: String(healerPick.poolNpcId) } });
    return r.status < 400;
  }
...
// Decide la accion del turno: si hay algun aliado (el actor mismo o un compañero, CURACION con
// target_type=ALLY puede apuntar a cualquiera) por debajo de ALLY_HEAL_HP_RATIO, cura al mas
// necesitado -- primero con skill si tiene mana para pagarla, si no con la mejor poción de HP que
// tenga en el inventario (USE_ITEM también acepta targetParticipantId, así que puede curar a un
// aliado con la poción igual que con la skill, no solo a sí mismo). Ya no huye nunca por HP bajo
// -- pedido explícito: se queda a pelear y curarse en vez de escapar. Si no hace falta curar,
// ataca con la skill mas cara que le alcance la mana (proxy simple de "mas fuerte", termina la
// pelea mas rapido que el golpe basico), o basico si no tiene ninguna.
async function decideCombatAction(playerId, actor, allies) {
...
async function playOutCombat(playerId, sessionId) {
  ...
  while (guard++ < MAX_FIGHT_ACTIONS) {
    ...
    const action = await decideCombatAction(playerId, actor, allies);
