...
// primero) y compra de a 3-4 publicaciones sueltas (el mercado vende cada publicación entera, ver
...
async function tryBuyPotionFromMarket(playerId, ctx) {
  ...
  for (const { code, statCode } of MARKET_POTIONS) {
    ...
    const buyCount = 3 + Math.floor(Math.random() * 2); // 3 o 4, a pedido explicito del usuario
    const listings = (await db.query(
      `SELECT id, price_per_unit FROM player_market_listings
       WHERE seller_id = $1 AND item_id = $2 AND status = 'ACTIVE' ORDER BY id LIMIT $3`,
      [sellerId, item.id, buyCount]
    )).rows;
    if (!listings.length) continue;

    let bought = false;
    for (const listing of listings) {
      if (Number(ctx.gold) < Number(listing.price_per_unit)) break;
      const r = await callRoute(H.marketBuy, playerId, { params: { id: String(listing.id) } });
      if (r.status < 400) {
        bought = true;
        ctx.gold = Number(ctx.gold) - Number(listing.price_per_unit);
      }
    }
