...
// Las 5 leyendas corren su tick EN PARALELO, no una atrás de otra -- no hay ningun dato
// compartido entre ellas (cada una tiene sus propias filas, propia sesion de combate, etc., ver
// legendTicksInFlight mas arriba para la traba que sí hace falta: la misma leyenda contra sí
// misma) y cada una usa como mucho una conexion del pool a la vez (son secuenciales puertas
// adentro), asi que 5 en paralelo son a lo sumo 5 conexiones simultaneas, dentro del limite
// default del pool (10). Promise.all en vez de Promise.allSettled porque cada error ya se atrapa
// individual adentro del map -- una leyenda que falle no tira abajo a las demas.
async function tickLegendSchedule() {
  const legends = await db.query('SELECT id FROM players WHERE is_bot = TRUE ORDER BY id');
  await Promise.all(legends.rows.map(async (row) => {
    try {
