...
async function processLegendTick(playerId) {
  ...
  if (Number(ctx.hp) <= 0 || Number(ctx.hp) / Number(ctx.max_hp) < CRITICAL_HP_RATIO) {
    await tryHeal(playerId, ctx);
    return;
  }
  ...
  if (await tryDailyEvent(playerId, ctx)) return;
  ...
  await exploreAndFight(playerId, ctx);
}
