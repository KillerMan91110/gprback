...
// Evalua una fila de class_evolution_requirements. COUNTER no tiene sistema de tracking todavia
// (ver db/schema.sql: no existe ninguna tabla de contadores de jugador), asi que queda marcado
// available=false en vez de met=false, para que el front pueda distinguir "no cumplido" de
// "todavia no se puede evaluar esto".
async function checkRequirement(playerId, req) {
  switch (req.requirement_type) {
    ...
    case 'ITEM': {
      ...
      return { met: compareValues(qty, req.comparison, target), available: true };
    }
...
async function evolvePlayer(playerId, evolutionId) {
  ...
  if (player.level < evo.required_level) {
    return { error: `Necesitás nivel ${evo.required_level} para evolucionar`, status: 400 };
  }
  ...
  for (const req of reqsResult.rows) {
    ...
    if (!checked.met) {
      return { error: `No cumplís el requisito: ${req.description}`, status: 400 };
    }
  }
...
module.exports = { getAvailableEvolutions, evolvePlayer, checkRequirement };
