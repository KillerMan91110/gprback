...
function resolveAttack(actor, target, elementalMods, categoryBonusPercent = 0) {
  ...
  // El basico de jugadores/NPCs siempre es FISICO (escala con ATK, mitiga contra DEF) -- asi
  // funciona en practicamente todo RPG, la magia de verdad sale de las skills (damage_school
  // MAGICO) o de la innata basic_attack_extra_mag_percent/imbue de mas abajo, no de "cual stat
  // sea mas alta". Los monstruos (actor.monster_code) conservan el auto-detect viejo: algunos
  // fueron diseniados como "caster" con base_magic_atk alto y esperan pegar con magia en su basico.
  const isMagic = actor.monster_code ? (actor.mag || 0) > (actor.atk || 0) : false;
  let offense = isMagic ? actor.mag || 0 : actor.atk || 0;
  if (!isMagic && actor.basic_attack_extra_mag_percent) {
    offense += (actor.mag || 0) * Number(actor.basic_attack_extra_mag_percent) / 100;
  }
  const rawMitigation = isMagic ? target.magic_def || 0 : target.def || 0;
  const mitigation = actor.ignore_def_percent ? rawMitigation * (1 - Number(actor.ignore_def_percent) / 100) : rawMitigation;
  ...
  if (target.is_defending) {
    if (!actor.ignores_defend_status) {
...
function resolveSkillHit(actor, target, skill, elementalMods, categoryBonusPercent = 0) {
  ...
  const effectiveMagicDef = elementalMods?.ignoreMagicDef ? 0 : (target.magic_def || 0);
  const rawMitigation = skill.damage_school === 'MAGICO'
    ? effectiveMagicDef
    : skill.damage_school === 'HIBRIDO'
      ? ((target.def || 0) + effectiveMagicDef) * 0.5
      : (target.def || 0);
  const mitigation = actor.ignore_def_percent ? rawMitigation * (1 - Number(actor.ignore_def_percent) / 100) : rawMitigation;
