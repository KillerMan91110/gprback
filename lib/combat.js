...
function resolveAttack(actor, target, elementalMods, categoryBonusPercent = 0) {
  ...
  const crit = Math.random() * 100 < (actor.crit_chance || 0);
  if (crit) {
  ...
  return { evaded: false, crit, damage };
}
...
function resolveSkillHit(actor, target, skill, elementalMods, categoryBonusPercent = 0) {
  ...
  const crit = elementalMods?.guaranteedCrit || Math.random() * 100 < (actor.crit_chance || 0);
  if (crit) damage *= 1 + (actor.crit_damage || 50) / 100;
  ...
  target.hp = Math.max(0, target.hp - damage);
  return { amount: damage, evaded: false, crit };
}
...
function resolveSkill(actor, targets, skill, elementalModsByTargetId = {}, categoryBonusByTargetId = {}) {
  return targets.map((target) => {
    ...
    return { target, amount: total, evaded: allEvaded, crit: anyCrit };
  });
}
