...
// Batched: una sola query para varios NPCs en vez de N (ver /party, /party/pool, /bench —
// antes llamaban getNpcEquipmentBonuses una vez por NPC en un Promise.all).
async function getNpcEquipmentBonusesBatch(npcIds) {
  const ids = [...new Set(npcIds)].filter(Boolean);
  const map = Object.fromEntries(ids.map((id) => [id, {}]));
  if (!ids.length) return map;

  const result = await db.query(
    `SELECT ne.npc_id, isb.stat_code, isb.amount, ne.enchant_level, ne.quality_tier
     FROM npc_equipment ne
     JOIN item_stat_bonuses isb ON isb.item_id = ne.item_id
...
module.exports = {
  ...
  getNpcEquipmentBonuses, getNpcEquipmentBonusesBatch, applyNpcHpBonusDelta,
